"""
OBT فروش (One Big Table) - wide_sales v4  (Optimized)
═══════════════════════════════════════════════════════
Changes from v3:
  + invoice_jalali_key  (int, format: 14050312)  — for 4 date fields
  + Start date filter: 1403/01/01  (2024-03-20)
  + Staging temp table + ON CONFLICT DO UPDATE  (eliminates DELETE bottleneck)
  + copy_expert with CSV + '|' delimiter  (safe for text fields with tabs)
  + Per-worker engine via Pool(initializer)  (no engine creation per chunk)
  + Nullable Int64 for INT_COLS  (preserves -1 as valid value)
  + Dedup within chunk before load
  + etl_updated_at explicitly in FINAL_COLS
"""
...
logger = setup_logger("obt_sales_v4")

# ─────────────────────────────────────────────────────────────
# ثابت‌ها
# ─────────────────────────────────────────────────────────────
PIPELINE_NAME = "obt_sales_v4"
TARGET_TABLE  = "wide_sales"
PK_COLS       = {"faktorsatr_id", "fiscal_year"}

# شمسی ۱۴۰۳/۰۱/۰۱  ≈  میلادی 2024-03-20
...
FINAL_COLS = [
    ...
    "product_type_code",
    "product_tax_eligible", "product_surcharge_eligible",
    "product_subsidy_eligible",
    ...
    "delivery_jalali_key",                                # ★ NEW

    # ── ETL ───────────────────────────────────────────────────
    "etl_updated_at",                                     # ★ NEW in FINAL_COLS
]
...
CREATE INDEX IF NOT EXISTS idx_ws_jalali_key      ON wide_sales (invoice_jalali_key);  -- ★ NEW
...
# ─────────────────────────────────────────────────────────────
# ساخت UPSERT SQL  (یک‌بار ساخته، چندین بار اجرا)
# ─────────────────────────────────────────────────────────────
def build_upsert_sql(columns: list) -> str:
    """
    INSERT INTO wide_sales (...) SELECT ... FROM tmp_staging
    ON CONFLICT (faktorsatr_id, fiscal_year) DO UPDATE SET ...
    """
    non_pk = [c for c in columns if c not in PK_COLS]

    # همه ستون‌ها بجز PK و etl_updated_at
    set_parts = [f"{c} = EXCLUDED.{c}" for c in non_pk if c != "etl_updated_at"]
    # etl_updated_at همیشه NOW()
...
def add_jalali_columns(df: pd.DataFrame, date_col: str, prefix: str) -> pd.DataFrame:
    ...
    def to_j(dt):
        ...
        try:
            ...
            jalali_key = int(f"{jd.year}{jd.month:02d}{jd.day:02d}")  # ★ 14050312
            return (
    ...
    df[f"{prefix}_jalali_key"]        = res.map(lambda x: x[5])      # ★ NEW
...
# ─────────────────────────────────────────────────────────────
# data quality validation (بدون تغییر)
# ─────────────────────────────────────────────────────────────
...
def save_failed_rows(failed_df: pd.DataFrame, chunk_start: int,
                     error_msg: str, tgt_engine):
    try:
        ...
        logger.warning(f"خطا در ذخیره dead letter: {str(e)[:100]}")


# ─────────────────────────────────────────────────────────────
# chunk log helpers (بدون تغییر)
# ─────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────
# ★ Worker Initializer — engine یک‌بار per process
# ─────────────────────────────────────────────────────────────
_worker_src = None
_worker_tgt = None
_worker_upsert_sql = None


def _worker_init(src_conn_str: str, tgt_conn_str: str, upsert_sql: str):
    """فراخوانی یک‌بار در هر worker process"""
    global _worker_src, _worker_tgt, _worker_upsert_sql

    _worker_src = create_engine(
        src_conn_str,
        pool_size=3, max_overflow=0,
        pool_pre_ping=True,
        fast_executemany=True,
    )
    _worker_tgt = create_engine(
        tgt_conn_str,
        pool_size=2, max_overflow=0,
        pool_pre_ping=True,
    )
    _worker_upsert_sql = upsert_sql
    logger.info(f"Worker PID={__import__('os').getpid()} — engines initialized")


# ─────────────────────────────────────────────────────────────
# ★ پردازش یک چانک — staging + upsert
# ─────────────────────────────────────────────────────────────
def process_chunk(args: tuple) -> dict:
    chunk_start, chunk_end, last_modified, chunk_size = args

    src_engine = _worker_src
    tgt_engine = _worker_tgt
    ...
    try:
        ...
        logger.info(f"  chunk={chunk_start} — executing SQL ({len(sql)} chars)")
        ...
        # ── تبدیل تاریخ‌های شمسی (شامل jalali_key) ──────────
        ...
        # ── تبدیل نوع داده‌ها (Int64 nullable) ────────────────
        for col in INT_COLS:
        ...
        chunk = chunk.drop_duplicates(
            subset=["faktorsatr_id", "fiscal_year"], keep="last"
        )
        if len(chunk) < before:
            logger.info(f"  Dedup: {before} → {len(chunk)} rows")

        # ── کیفیت داده ────────────────────────────────────────
        is_ok, issues = validate_chunk(chunk, chunk_start)
        if issues:
            logger.warning(f"  چانک {chunk_start}: مشکلات کیفیت: {issues}")
        ...
        try:
            with raw_conn.cursor() as cur:
                # ★ Temp staging table — بدون constraint, بدون index
                cur.execute("""
                ...
                # ★ COPY با CSV format و delimiter امن
                copy_sql = (
                ...
                # ★ UPSERT — حذف DELETE کامل
                cur.execute(_worker_upsert_sql)
...
# ★ Pipeline اصلی v4
# ─────────────────────────────────────────────────────────────
def run_obt_sales_pipeline_v4(
    chunk_size:  int = 100_000,
    max_workers: int = 4,
    max_rows:    int = None
):
    logger.info("=" * 60)
    logger.info(f"OBT فروش v4 — wide_sales "
    ...
    try:
        ...
        if last_run and last_run.get("last_from_value"):
            ...
            logger.info(f"ادامه از checkpoint: {last_modified}")
        else:
            last_modified = JALALI_1403_START
            logger.info(f"🆕 لود از ابتدای ۱۴۰۳: {last_modified}")
        ...
        try:
            ...
            logger.warning("keyset query ناموفق — استفاده از range ساده")
            keyset_ids = list(range(0, total_est * 100, chunk_size))

        # ── ساخت آرگومان‌ها ───────────────────────────────────
        all_args = []
        for i, kid in enumerate(keyset_ids):
            end_id = (keyset_ids[i + 1] - 1
                      if i + 1 < len(keyset_ids)
                      else kid + chunk_size)
            all_args.append((kid, end_id, last_modified, chunk_size))

        total_chunks = len(all_args)
        logger.info(f"{total_chunks} چانک — شروع multiprocessing...")

        # ── ★ ساخت UPSERT SQL (یک‌بار) ──────────────────────
        upsert_sql = build_upsert_sql(FINAL_COLS)

        # ── ★ Connection string ها برای worker initializer ────
        src_conn_str = (
            f"mssql+pyodbc://{extractor.src_engine.url.username}:"
            f"{quote_plus(extractor.src_engine.url.password)}"
            f"@{extractor.src_engine.url.host}:"
            f"{extractor.src_engine.url.port or 1433}/"
            f"{extractor.src_engine.url.database}"
            f"?driver=ODBC+Driver+17+for+SQL+Server"
        )
        tgt_conn_str = (
            f"postgresql://{loader.tgt_engine.url.username}:"
            f"{loader.tgt_engine.url.password}"
            f"@{loader.tgt_engine.url.host}:"
            f"{loader.tgt_engine.url.port or 5432}/"
            f"{loader.tgt_engine.url.database}"
        )

        # ── اجرا با multiprocessing ───────────────────────────
        total_rows   = 0
        total_errors = 0
        completed    = 0
        ...
        with Pool(
            ...
            initargs=(src_conn_str, tgt_conn_str, upsert_sql),
        ) as pool:
            for res in pool.imap_unordered(process_chunk, all_args):
                ...
                if res["status"] == "FAILED":
                    ...
                    logger.warning(
                        f"چانک {res['chunk_start']}: کیفیت رد شد"
                    )
                else:
                    progress = completed / total_chunks * 100
                    elapsed  = (datetime.now() - start_time).total_seconds()
                    eta_min  = ((elapsed / completed * (total_chunks - completed)) / 60
                                if completed > 0 else 0)
                    logger.info(
                        f"{completed}/{total_chunks} ({progress:.0f}%) | "
                        f"درج={res['rows_inserted']:,} | "
                        f"مجموع={total_rows:,} | ETA: {eta_min:.0f} min | "
                        f"chunk={res['duration_sec']:.1f}s"
                    )
        ...
        ║ speed:    {speed:>12,.0f} r/s  ║
        ║ time:     {duration/60:>12.1f} min  ║
        ║ status:   {final_status:>15}  ║
        ╚══════════════════════════════════════════╝
        """)

        if total_errors > 0:
            alerter.alert_error(PIPELINE_NAME,
                                f"{total_errors} چانک ناموفق", total_rows)
        return total_rows

    except Exception as e:
        logger.error(f"خطای کلی: {str(e)}", exc_info=True)
        checkpoint.save_checkpoint(PIPELINE_NAME, "FAILED",
...
if __name__ == "__main__":
    run_obt_sales_pipeline_v4(chunk_size=100_000, max_workers=4, max_rows=None)
